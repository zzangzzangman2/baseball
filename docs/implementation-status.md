...
| 내부 타석 v1 | partial | `docs/fmkorea-gm-source-map.md`, `docs/system-blueprint-index.md`, `src/engine.js` | 타석 단위 결과가 `plateAppearanceEvents`로 최근 경기 결과에 남고, 좌우 split, KBO 구장 성향, 포지션 그룹 수비, 실책 출루, 병살이 박스스코어/시즌 기록에 반영된다. | 볼카운트/투구 수 입도, 포수 프레이밍, 교체/작전 AI, 전체 시즌 원장화. |

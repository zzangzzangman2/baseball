...
- 투명 알파가 이미 포함된 계약 소스는 그 알파 매트를 그대로 신뢰한다. 체크무늬 제거를 다시 적용해 밝은 유니폼이나 목 연결 픽셀을 지우지 않는다.
...
각 완성 프레임의 두 번째 알파 연결 요소는 최대 3px까지만 허용한다. 이보다 큰 분리 영역은 머리·몸·장비가 끊어진 것으로 보고 검증을 실패시킨다.

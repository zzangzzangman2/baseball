...
function checkGamecastMotionAtlas() {
  ...
  assert(safeInteger(meta.nativeDisplaySize?.w) === 80 && safeInteger(meta.nativeDisplaySize?.h) === 80 && safeInteger(meta.nativeRenderScale) === 1, `native display 메타가 80px/1x가 아닙니다: ${JSON.stringify(meta.nativeDisplaySize)}/${meta.nativeRenderScale}`, GAMECAST_PLAYER_ATLAS_PATH);
  ...
  for (const [fileName, lighting] of GAMECAST_PLAYER_ATLAS_VARIANTS) {
    ...
    assert(safeInteger(variantMeta.nativeDisplaySize?.w) === 80 && safeInteger(variantMeta.nativeDisplaySize?.h) === 80 && safeInteger(variantMeta.nativeRenderScale) === 1, `${fileName} native display가 80px/1x가 아닙니다.`, atlasPath);
